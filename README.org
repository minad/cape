...
#+html: <a href="https://releases.melpa.org/#/cape"><img alt="MELPA Releases" src="https://releases.melpa.org/packages/cape-badge.svg"/></a>
#+html: <a href="https://snapshots.melpa.org/#/cape"><img alt="MELPA Snapshots" src="https://snapshots.melpa.org/packages/cape-badge.svg"/></a>
...
Cape is available on [[https://elpa.gnu.org/packages/cape.html][ELPA]] and [[https://releases.melpa.org/#/cape][MELPA]]. You can install the package with
...
after another, for example you can use ~cape-file~ together with programming mode
...
this sorting behavior use ~cape-capf-sort~, such that the completion UI can apply
its own sorting.
...
~company--multi-backend-adapter~ from Company. In order to combine multiple Capfs,
such that each of the Capfs is tried one after another, use ~cape-capf-choose~.
...
- ~cape-capf-trigger~, ~cape-wrap-trigger~: Create a Capf with a trigger prefix character.
